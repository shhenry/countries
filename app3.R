...
# youngs modulus for steel cable times the cross sectional area of the wire
T <- (2.10*(10^11))*0.1016
...
server <- function(input, output, session) {
  ...
  observeEvent(input$compute, {
    ...
    rk <- rungeKutta(t0 = 0, z0= c(0,0,0,input$y0), L = input$L,
                     stepSize = input$n, fns = res$system)
    res$position <- data.frame(x = rk$df$system)
    #res$stuck <- rk$stuck
